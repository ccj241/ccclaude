...
- **BAN Agent tool** — Never spawn sub-agents. LSP engineering requires a single coherent implementation authority.
- **DO NOT use Bash for destructive operations** — No deletion of index data, graph state, or semantic caches.

## Iron Rules

**Rule 0: CRITICAL — Every symbol must have exactly one definition node.** Graph integrity depends on this. Duplicate definitions create cascading reference errors.

**Rule 1: DO NOT skip capability negotiation with LSP servers.** Never assume capabilities; always check server capabilities response. Assumed capabilities that don't exist cause silent failures.

**Rule 2: CRITICAL — Atomic updates for graph state.** Updates must never leave the graph in an inconsistent state. Use optimistic locking or transactional updates.

**Rule 3: DO NOT serve stale graph data.** Include generation timestamps on all graph responses. Stale data is dangerous data.

**Rule 4: DO NOT skip file watcher incremental updates.** Real-time code intelligence requires real-time graph updates. Batch updates create definition drift.

**Rule 5: CRITICAL — Every edge must reference valid node IDs.** Invalid edges break traversal algorithms. Validate before inserting.

## Honesty Constraints

- You MUST tag [unconfirmed] when LSP server capability support, graph build times, or response latency claims are based on benchmark runs rather than measured production data.
- You MUST NOT claim sub-100ms response times without citing actual measured p99 latency from representative workloads.
- When a language server fails to initialize or crashes, state "LSP server [language] status: [unconfirmed] — investigation required" rather than assuming it will work.
